...
\textbf{Changes:}
\begin{itemize}[leftmargin=2em]
  \item A new notebook \texttt{notebooks/14\_camb\_benchmark.ipynb} computes the Planck-2018 best-fit $D_\ell^{\rm TT}$ via CAMB and compares it to the DSC sky-map spectrum from a 5-seed ensemble.
...
\textbf{Changes:}
\begin{itemize}[leftmargin=2em]
  \item In Block 3 (\texttt{sec:camb}, and in the new notebook 14) we use the flat-sky / Limber projection $\ell \approx 2\pi R_{\rm shell}\,k$ to translate lattice peaks to multipoles.
...
\textbf{Notebooks (cumulative count: 28).}
\texttt{notebooks/14\_camb\_benchmark.ipynb} (CAMB Planck-2018 forward benchmark);
\texttt{notebooks/15\_alpha\_drift\_quasar.ipynb} (Pillar A original);
\texttt{notebooks/16\_jwst\_h0\_anchors.ipynb} (Pillar C 4-anchor);
\texttt{notebooks/17\_quasar\_293\_fit.ipynb} (Pillar B King 2012 full sample);
\texttt{notebooks/18\_camb\_fisher.ipynb} (Pillar D);
\texttt{notebooks/19\_late\_time\_stability.ipynb} (Pillar E);
\texttt{notebooks/20\_pillar\_F\_cosmic\_chronometers.ipynb};
\texttt{notebooks/21\_pillar\_G\_pantheon.ipynb};
\texttt{notebooks/22\_pillar\_H\_desi\_dr2.ipynb};
\texttt{notebooks/23\_pillar\_J\_joint.ipynb};
\texttt{notebooks/24\_pillar\_K\_subset\_scan.ipynb};
\texttt{notebooks/25\_murphy03\_replication.ipynb} (Pillar K Murphy + King decomposition);
\texttt{notebooks/26\_pillar\_M\_leverage.ipynb};
\texttt{notebooks/27\_pillar\_N\_atomic\_clock.ipynb};
\texttt{notebooks/28\_pillar\_O\_forecast.ipynb}.
