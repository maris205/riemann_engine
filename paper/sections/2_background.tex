...
The numerical spectral observation that period-doubling renormalization flow exhibits a level-counting law of the same logarithmic form~\citep{Wang2026Spectral} provides additional motivation for adopting \cref{eq:rvm} as the working ansatz.
...
For the undamped ($\eta = 0$) case used in the acoustic-peak experiments (Supplementary Material), the evolution is \emph{exactly} symplectic.
...
We note that the formal divergence of $r_s$ distinguishes the DSC mechanism from standard recombination, where the integral converges; the ``finite horizon'' reported in the Supplementary Material refers to this effective, threshold-dependent quantity.
